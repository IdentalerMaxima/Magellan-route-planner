...
public class DBTablePrinter {
     ...
     * @param maxStringColWidth Max. width of text columns
     */
    public static <ResultSetMetaData> void printResultSet(ResultSet rs, int maxStringColWidth) {
        try {
            if (rs == null) {
            ...
            int columnCount = ((java.sql.ResultSetMetaData) rsmd).getColumnCount();
            ...
            for (int i = 1; i <= columnCount; i++) {
                Column c = new Column(((java.sql.ResultSetMetaData) rsmd).getColumnLabel(i),
                        ((java.sql.ResultSetMetaData) rsmd).getColumnType(i), ((java.sql.ResultSetMetaData) rsmd).getColumnTypeName(i));
                c.setWidth(c.getLabel().length());
                c.setTypeCategory(whichCategory(c.getType()));
                columns.add(c);

                if (!tableNames.contains(((java.sql.ResultSetMetaData) rsmd).getTableName(i))) {
                    tableNames.add(((java.sql.ResultSetMetaData) rsmd).getTableName(i));
                }
            }
